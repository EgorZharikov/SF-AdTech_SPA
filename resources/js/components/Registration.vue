    ...
    <div class="row account-card align-items-center" :style="{ height: 100 + 'vh' }">
        <div class="col col-md-4 offset-md-4">
            ...
            <div class="mb-3">
                ...
                <input v-model="password_confirmation" type="password" name="password_confirmation" class="form-control" id="password_confirmation">
            </div>
            <div class="d-grid gap-2">
                <button @click.prevent="register" type="submit" name="signup" id="signIn" class="btn btn-primary">Отправить</button>
            </div>
            <div v-if="errors" class="alert alert-danger mt-3" role="alert"><p v-for="error in errors">{{ error.toString()}}</p>
            </div>
        </div>
    </div>
</template>
